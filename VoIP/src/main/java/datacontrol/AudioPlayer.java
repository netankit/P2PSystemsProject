...
public class AudioPlayer extends Thread {
	...
	Log logger = lg.getLog(AudioPlayer.class.getName());

	private AudioFormat audioFormat;
	private SourceDataLine sourceDataLine; // sound device source line
	/**
	 * Take the block from this queue and play it. AudioPlayer is consumer,
	 * RTPReceiver is producer.
	 */
	private BlockingQueue<AudioDataBlock> CarrierQueue;
	/**
	 * After playing, return the block into this queue. AudioPlayer is producer,
	 * RTPReceiver is consumer.
	 */
	...
	public void run() {
		try {
			while (true) {
				AudioDataBlock block = CarrierQueue.take(); // first take audio
															// block and play it
