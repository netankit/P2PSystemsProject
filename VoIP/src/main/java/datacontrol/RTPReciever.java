...
public class RTPReciever implements RTPAppIntf {
	...
	Log logger = lg.getLog(RTPReciever.class.getName());
	...
	// Audio block is received, RTPReceiver first obtain a blank block from this
	// queue and then write the block.
	...
	public RTPReciever(VOIP voip, int rtpPort, BlockingQueue<AudioDataBlock> carrierQueue,
			BlockingQueue<AudioDataBlock> returnQueue) {
		this.voip = voip;
		RTPPort = rtpPort;
		int rtcpPort = RTPPort + 1; // port for RTCP communication is always the
									// next one
	...
	public void receiveData(DataFrame frame, Participant participant) {
		byte[] recievedPacket = frame.getConcatenatedData(); // obtain the audio
																// bytes
		...
		System.arraycopy(data, 0, block.data, 0, block.dataBlockSize); // copy
																		// the
																		// audio
																		// data
	...
	public void stopReceiver() {
		...
		try {
			block = ReturnQueue.take();
			block.actualDataBlockSize = -1; // notify the AudioPlayer that there
											// is no audio data
